      ...
      <div className="bg-white p-5 rounded-3xl border border-[#BFE5D3] shadow-sm flex flex-col justify-between h-full min-h-[380px] space-y-4 hover:border-[#1F8F68] transition-colors">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold text-[#12372A]">Housing Snapshot</h3>
              <span className="bg-[#EAF7F1] text-[#1F8F68] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                LIVE
              </span>
            </div>
            <div className="flex items-center gap-1.5 text-xs text-slate-500">
              ...
              <button
                ...
                className="p-1 rounded-lg bg-[#F3FBF7] hover:bg-[#EAF7F1] text-[#1F8F68] border border-[#BFE5D3] transition-all cursor-pointer"
                title="Edit Housing Rent Values (Pen Icon)"
              >
                <Pencil className="w-3.5 h-3.5" />
              </button>
              <Info className="w-3.5 h-3.5 text-[#1F8F68] cursor-pointer" />
            </div>
          </div>

          {/* Inline Pen Customizer */}
          {isEditMode && (
            <div className="bg-[#F3FBF7] p-2.5 rounded-2xl border border-[#BFE5D3] space-y-1.5 text-xs">
          ...
          <div className="flex items-center gap-1 p-1 bg-[#F3FBF7] rounded-xl border border-[#BFE5D3] text-xs font-semibold text-slate-600">
            <button
              onClick={() => setActiveMode('Rent')}
              className={`flex-1 py-1.5 rounded-lg transition-all text-center cursor-pointer ${
                activeMode === 'Rent' ? 'bg-[#1F8F68] text-white shadow-xs font-extrabold' : 'hover:text-[#12372A]'
              }`}
            >
              Rent
            </button>
            <button
              ...
              className={`flex-1 py-1.5 rounded-lg transition-all text-center cursor-pointer ${
                activeMode === 'Buy' ? 'bg-[#1F8F68] text-white shadow-xs font-extrabold' : 'hover:text-[#12372A]'
              }`}
            >
              Buy (Mortgage)
            </button>
            <button
              ...
              className={`flex-1 py-1.5 rounded-lg transition-all text-center cursor-pointer ${
                activeMode === 'Room' ? 'bg-[#1F8F68] text-white shadow-xs font-extrabold' : 'hover:text-[#12372A]'
              }`}
            >
              Room / Dorm
            </button>
          ...
          <div className="flex items-center justify-between text-xs font-semibold pt-1">
            <span className="text-[#12372A] font-extrabold text-xs">
            ...
            <div className="flex items-center gap-1.5">
              <select
                ...
                className="bg-[#F3FBF7] border border-[#BFE5D3] rounded-lg px-2 py-1 text-xs text-[#12372A] font-bold focus:outline-none focus:ring-2 focus:ring-[#1F8F68]"
              >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-[#BFE5D3]/60 text-slate-400 font-semibold text-[11px]">
            ...
            <tbody className="divide-y divide-slate-50">
              {dynamicNeighborhoods.map((n, idx) => (
                <tr key={n.id} className="hover:bg-[#F3FBF7]/80 transition-colors">
                  <td className="py-2 px-1 font-bold text-[#12372A] flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded-full bg-[#EAF7F1] text-[#1F8F68] text-[10px] font-bold flex items-center justify-center shrink-0">
                      {idx + 1}
                    </span>
                    <span className="truncate max-w-[100px]">{n.name}</span>
                  </td>
                  <td className="py-2 px-1 font-extrabold text-[#12372A] whitespace-nowrap">
                    {formatCurrency(n.typicalRate, inputs.currency)}/mo
                  </td>
                  <td className="py-2 px-1 font-bold whitespace-nowrap">
                    {n.vsLastMonth < 0 ? (
                      <span className="text-[#1F8F68]">↓ {Math.abs(n.vsLastMonth)}%</span>
                    ) : (
                      <span className="text-[#1F8F68]">↑ {n.vsLastMonth}%</span>
                    )}
                  </td>
        ...
        <div className="flex items-center justify-between border-t border-[#BFE5D3]/60 pt-3 text-xs">
          ...
          <div className="flex items-center gap-1 text-[11px] text-slate-400">
            ...
            <RefreshCw className="w-3.5 h-3.5 text-[#1F8F68] cursor-pointer" />
          </div>
        </div>
      </div>
